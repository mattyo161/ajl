...
def test_plan_dedupes_profiles_by_account(monkeypatch):
    ...
    sessions = fanout.plan_sessions(runner, opts(all_profiles=True))
...
def test_plan_expands_regions(monkeypatch):
    ...
    monkeypatch.setattr(fanout, "enabled_regions",
                        lambda r, sk: ["us-east-1", "us-west-2", "eu-west-1"])
    sessions = fanout.plan_sessions(runner, opts(all=True))
...
def test_run_fanout_contains_failures(monkeypatch):
    ...
    monkeypatch.setattr(fanout, "enabled_regions", lambda r, sk: ["us-east-1"])
    out = io.StringIO()
    ...
    code = fanout.run_fanout(runner, emitter, opts(all_profiles=True), run_one)
